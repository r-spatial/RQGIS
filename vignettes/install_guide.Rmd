...
The `osgeo4mac` tap supports the most recent QGIS versions for both the development version and the long term release (LTR). You can compile with various options, e.g. `--with-grass` or `--with-saga-gis`. Check `brew info qgis2` for all available options. We recommend to compile QGIS using 

`brew install qgis2 --with-grass --with-saga-gis`. 

The installation will take some time (30-45 mins, depending on your system). 
If you want to install the LTR version, simply replace `qgis` by `qgis2-ltr`. 
...
Using homebrew, SAGA can be compiled via `brew install saga-gis`. If you installed QGIS using option 1) from the QGIS Mac section of this document, you will already have SAGA installed.
