...
![](../inst/figures/05_command_line_utilties.PNG)
...
If you do not want to use `homebrew`, you can install the QGIS binary from [https://www.qgis.org/en/site/forusers/download.html](https://www.qgis.org/en/site/forusers/download.html). In fact, this is exactly doing the same as `brew cask install qgis`.

If you want to build QGIS from source, please use the 'osgeo4mac' tap of `homebrew` with `brew install qgis2`. 
Note: Right now, issues with the `processing` module occur when installing QGIS this way, so we recommend to use the other approach until this is fixed.
...
There is no binary install of SAGA for Mac. Therefore, we recommended to use bottle compiling with `homebrew`. Alternatively, you can compile SAGA from source from the [SAGA website](https://sourceforge.net/projects/saga-gis/files/). Make sure to check RQGIS compatibility with your SAGA version! You can do so using `qgis_session_info()` in RQGIS.
