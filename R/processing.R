...
run_qgis <- function(alg = NULL, ..., params = NULL, load_output = FALSE,
  ...
  # be unquoted which can only be achieved in R by collapsing all arguments into
  ...
  vals <- vapply(params, function(x) {
    ...
    # shellQuote argument if is not True, False, None or raster or vector
    ifelse(grepl("True|False|None|[r|v]layer", tmp), tmp, shQuote(tmp))
  }, character(1))
