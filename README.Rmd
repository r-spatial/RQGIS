...
## QGIS modifications (v. 2.16-2.18.1)
...
The QGIS core team fixed this bug, and starting with QGIS 2.18.2 this manual adjustment is no longer necessary [post](http://gis.stackexchange.com/questions/204321/qgis-2-16-processing-runalg-fails-when-run-outside-of-qgis-in-a-custom-applicat).
...
- we should rewrite execute_cmds so that it can also be used with qgis_session_info, open_help and get_args_man. This would delete rendundany and make the code more consistent. In this case, we could have had rewritten the python py_cmd.py to paste("python", file.path(tmp_dir), "py_cmd.py")) in execute_cmds
