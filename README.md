...
In order to run RQGIS properly, you need to download various third-party software packages. To fascilitate the download and installation procedures for each platform (Windows, Linux, Mac OSX), we wrote a vignette. To access it, use `vignette("install_guide", package = "RQGIS")`. Overall, we recommend to use the current LTR of QGIS (2.14) with RQGIS. Please note that RQGIS so far does **not support** the latest QGIS release, **QGIS 2.16** Nødebo.
...
To install the `devtools` package along with its dependencies under **Linux** (Ubuntu), run following lines in the terminal:
